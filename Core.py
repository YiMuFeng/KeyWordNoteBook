...
class KeyWordNoteBook:
    ...
    def _init_or_load_file(self):
        ...
        if missing:
            raise RuntimeError(f"文件参数不完整，缺少：{', '.join(missing)}")
        ...
        try:
            ...
            raise RuntimeError("登录时输入的密码不正确")  # 终止初始化
        except exceptions.VerificationError:
            raise RuntimeError("登录时哈希字符串格式错误")
        except Exception as e:
            # 捕获非预期错误
        ...
        if computed_hmac != params["integrity_check"]:
            raise RuntimeError("文件HMAC校验失败，内容可能被篡改或损坏")
...
if __name__=="__main__":
    m_KeyWordNotBook = KeyWordNoteBook("testpw")

    # # 新建条目
    testlint = KeyItem()        # 类似于 testlint ={}
    testlint.update({"Index": "3",
        ...
        "Note": "测试" })
    # # 写入密码本
    m_KeyWordNotBook.delete_item("7",upw="testpw")
